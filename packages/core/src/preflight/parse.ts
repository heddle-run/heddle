...
/**
 * What an agent needs from the machine, declared — the half of preflight that
 * reads and renders declarations without looking at any machine.
 *
 * Split from the check half (`check.ts`) so a host with no filesystem can
 * still read a bundle's `requires` and say what it asks for: parsing, labels
 * and report formatting are pure string work, and they are what a portable
 * host needs. Everything that stats, walks `$PATH` or reads `process.env`
 * lives next door.
 *
 * `hint` follows the same rule the whole feature does: it is a string shown
...
export interface CheckedRequirement {
   ...
   * read out of the environment — see the check half's `checkEnv`.
   */
  found?: string;
}
 ...
 *
 * Both separators are path-ish on some machine this string may describe, and
 * this half of preflight refuses to ask the machine it is on — so the rule is
 * about the string, not about `path.sep`.
 */
...
export function shortName(candidate: string): string {
...
export function parseNodeRange(
  range: string,
