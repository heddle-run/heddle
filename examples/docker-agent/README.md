...
(cagent's YAML configuration) — not another encoding of Agent Spec, but a
different schema with different ideas: named agents, a models table, an
...
Running it is the same flow any Agent Spec file produces — an OpenAI key, a
...
would import a YAML library and delete that half of the file. `parse` is yours
— whatever turns text into an Agent Spec document is a valid format.
