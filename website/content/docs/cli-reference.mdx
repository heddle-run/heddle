...
|----------|-------------|
| `flow` | Path to the flow file (JSON or YAML, detected by extension), a `.heddle` bundle made by [`heddle bundle`](#heddle-bundle), an `https://` address of one, or a bare name from the [library](/library): `heddle run coding-agent` fetches and runs `https://heddle.run/library/coding-agent.heddle`. A file of the same name wins over the shorthand |
...
| `--input <json>` | Input data as a JSON object, filling the flow's declared `inputs` |
...
The run's answer — the payload of the outcome it ended at, plus `outcome` naming it — is
printed to stdout as formatted JSON, unless `--protocol` is given, which replaces it with
a frame stream. Progress, tool activity, and errors go to stderr, so stdout stays
pipeable:
...
`--input` is checked against the flow's declared `inputs` at the first node: a declared
input you omit fails the run at once, unless it carries a `default`, which is filled in.
A key the document does not declare is carried but unreachable, since every `{{inputs.…}}`
reference was already checked against the declaration when the document loaded.
...
Your message is bound to the first field declared under the flow's `inputs`, falling back to `query` if it declares none. Type `/exit` or `/quit` to leave.
...
```bash
heddle bundle weave.yaml --tools-dir tools --plugin plugin.json \
...
| `-o`, `--output <file>` | Where to write the bundle. Default: `<flow name>.heddle`, named for the flow rather than the file, since flow files are usually all called `weave.yaml` |
...
1. The document's own shape: the `weave` version, the verbs, the schemas, and every key —
   an unknown key is an error anywhere outside `meta` and a plugin step's `with`
2. Every reference: models, tools and step targets resolve, every `{{…}}` names a real
   producer and a key it actually writes, and control flows forward on every path
3. Plugin components, against the schema their plugin registered — which is why a flow
   naming a plugin's component types needs that `--plugin` here too
4. That every tool the flow names resolves, against the composed registry, so this runs
...
therefore validating it, never reaches a credential. A spec whose model config names
...
There is no partial verdict. A spec that does not parse exits 1, so does a bad reference,
a component type no loaded plugin provides, and a tool the flow names that nothing
carries. `Valid` printing and exit 0 mean the whole document was checked and the flow will
start; there is no `Graph validation skipped` — a check that cannot run is a failed check.
...
Creates a project directory containing a template `weave.yaml` and a sample tool in `tools/`. Existing files at those paths are overwritten.
