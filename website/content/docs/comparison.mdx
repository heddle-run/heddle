...
description: How Heddle compares to LangChain, CrewAI, AutoGen, and other popular AI agent frameworks.
---
...
|-----------|----------|-----------------|---------|
| [Heddle](https://github.com/spichen/specrun) | Any (tools are executables) | Declarative (YAML/JSON) | MIT |
...
|-----------|---------------------|------------------|
| Heddle | Any OpenAI-compatible, vLLM, Ollama | Standalone executables (stdin/stdout), any language |
...
|-----------|-------------|-----------------|
| Heddle | Yes (multi-node flows) | Local CLI |
...
|-----------|---------------|--------------|
| Heddle | Low | Declarative specs, language-agnostic tools, no backend needed |
...
## When to Choose Heddle

- **You want to define workflows as data, not code** — Heddle workflows are YAML/JSON files, not Python scripts.
...
- **You want to run agents locally** — A single `heddle run` command compiles, validates, and executes your workflow. No backend or service required.
