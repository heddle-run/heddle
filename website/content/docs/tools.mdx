...
<Callout type="info">
Plugins loaded with `--plugin` are not sandboxed: the CLI imports the module into the heddle process, where it runs with full Node privileges. Only the tools it invokes via `ctx.runTool` are confined. Plugins submitted to the engine as a manifest plus source are different — each runs in its own process with an empty environment, and is confined by the same sandbox as tools when one is configured.
</Callout>
