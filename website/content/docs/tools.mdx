...
<Callout type="info">
Sandboxing applies to tools, not to plugins: plugin modules are imported into the heddle process and run with full Node privileges. Only the tools they invoke via `ctx.runTool` are confined.
</Callout>
