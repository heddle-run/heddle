...
description: Embed the engine with @heddle-run/core. Load, compile, validate and run a flow from your own program.
---
...
- **The library** (`@heddle-run/core`) embeds a flow in a program of your own: a
...
flow stays a plain Weave document either way; what you are importing is the runtime,
...
`@heddle-run/core` needs Node.js 18 or newer, and is ESM.
...
Scaffold something to run (`npx @heddle-run/cli init my-agent` gives you a `weave.yaml` and a
...
const flow = loadFlow("my-agent/weave.yaml");
...
- **`loadFlow(path)`** reads JSON or YAML (detected by extension) and runs both
  validation passes: the document's own shape, then every reference, target and plugin
  component. A flow naming a plugin's component types needs the plugins at load
...
- **`validate(graph)`** is the backstop over heddle's own lowering: it throws on a
  compiled graph with an unreachable node or a dead end. A document `loadFlow` accepted
  will pass it; keep the call anyway, as `heddle validate` itself does.
- **`runner.run(signal, inputs, from?)`** walks the graph and resolves to the final
  ...
  the CLI: a plain object filling the flow's declared `inputs`. `from` resumes a
...
| `allowEnvRefs` | Whether `$VAR` in a spec's model config is resolved from the process environment. Defaults to true; a host running *submitted* specs sets it false |
| `defaultLlmKey`, `defaultLlmUrl` | Fallback credential and base URL for a spec whose model config carries neither, which is what the server's default-credential flags feed |
...
| `tool_call` | An agent or `tool` step invokes a tool, with `toolName` and `toolArgs` |
