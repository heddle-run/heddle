...
Default-export a plain object. Nothing to import: a plugin depends on heddle's shapes,
not its code, which is how every shipped example is written.
...
`name` and `version` are not decorative: a document that pins a component type with
top-level `requires` is checked against `version`, so a flow can say which release of
your plugin it was written for.
...
| `callModel(request)` | node, transform, middleware | Ask this component's `model` config for an answer |
...
[workspace](/docs/tools#the-workspace), on every run, sandbox or not, so a path from
...
A transform hangs off an agent's `transforms` and sees the agent's messages: `pre` on their
...
`transform_status: "rejected"`, which a builtin [`switch` step](/docs/nodes#switch) can
route on.

## A provider

A provider answers model calls itself, so a spec writes your component type as a model's
`provider`, beside the builtins. You *are* the endpoint, so you receive the whole request
including `model`.
...
`ctx.component` is the model config the spec wrote, which is where a key or a url the
...
A plugin cannot take a builtin provider's name. A flow writing `provider: openai`
reaches heddle's own client whatever plugins are loaded.
...
second, and heddle kills what has not exited by then, so this is where a connection
closes rather than where a backlog drains.
