...
Default-export a plain object. No import — a plugin depends on heddle's shapes, not its
code, which is how every shipped example is written.
...
For TypeScript, or an editor that will use it, `@heddle/core` exports `definePlugin` — an
...
[workspace](/docs/tools#the-workspace) — every run, sandbox or not — so a path from
