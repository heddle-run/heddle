...
Flows are written in JSON or YAML following the [Open Agent Specification](https://oracle.github.io/agent-spec/). Field names in the file are `snake_case`.
...
Graph validation checks that every node is reachable from the start node, that a reachable `EndNode` exists, and that no non-terminal node is a dead end. It does not yet check that branch names on edges line up with a `BranchingNode` mapping, nor that data flow edges reference properties the nodes actually declare.
